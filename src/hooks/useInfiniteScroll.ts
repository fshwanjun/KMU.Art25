...
export function useInfiniteScroll(options: UseInfiniteScrollOptions = {}): UseInfiniteScrollReturn {
  ...
  const loadNext = useCallback(() => {
    console.log("[useInfiniteScroll] loadNext 호출");
    setSectionIds((prev) => {
      const lastId = prev[prev.length - 1];
      const newId = lastId + 1;
      const newIds = [...prev.slice(1), newId];
      console.log("[useInfiniteScroll] sectionIds 업데이트:", {
        prev,
        newIds,
        removed: prev[0],
        added: newId,
  ...
  useIntersection(
    ...
    (entry: IntersectionObserverEntry) => {
      if (disabled) {
        console.log("[useInfiniteScroll] disabled 상태로 인해 트리거 무시");
        return;
      }
      ...
      if (entry.isIntersecting) {
        ...
        console.log("[useInfiniteScroll] IntersectionObserver 트리거:", {
        ...
        if (isTriggerPoint && !triggeredRef.current) {
          console.log("[useInfiniteScroll] 트리거 조건 만족! loadNext 호출");
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (isTriggerPoint) {
        console.log("[useInfiniteScroll] 스크롤 백업 트리거! loadNext 호출");
        triggeredRef.current = true;
        loadNext();
      }
    };
